...
function cadastrarParticipante() {
    ...
    function fixStepIndicator(n) {
        var i,
            x = document.getElementsByClassName("session-item");

        for (i = 0; i < x.length; i++) {
            if (i > n) {
                x[i].classList.remove("item-checked");
            } else if (i < n) {
                x[i].classList.add("item-checked");
            }
        }

        x[n].classList.add("item-checked");
    }
}
...
function requestService(tipo, searchString, page) {
    var url = "http://localhost:8080/" + tipo + "/?titulo=" + searchString + "&pagina=" + page;
    console.log(url);
...
function updateTable(data, tbody) {
    ...
    if (content.length == 0) {
        ...
        content.map(function (chamada) {
            ...
            var row = "<tr>\n              <td><a href=\"" + urlId + "\" class=\"has-text-weight-medium\">" + chamada.titulo + "</a></td>\n              <td><span class=\"" + (chamada.status !== 'FECHADO' ? 'tag is-spaced is-rounded is-primary' : 'tag is-spaced is-rounded is-black') + "\">" + (chamada.status == 'EMANDAMENTO' ? chamada.status.slice(0, 1) + '' + chamada.status.slice(1, 2).toLowerCase() + ' ' + chamada.status.slice(2).toLowerCase() : chamada.status.slice(0, 1) + '' + chamada.status.slice(1).toLowerCase()) + "</span></td>\n              <td><strong>" + formatData(chamada.dataInicio) + "</strong> at\xE9 <strong>" + formatData(chamada.dataTermino) + "</strong>\n              <td>41/50</td>\n              <td><a href=\"" + urlId + "\" class=\"has-text-weight-medium\">Acessar chamada</a></td>\n          </tr>";
            tbody.innerHTML += row;
        });
    }
}
